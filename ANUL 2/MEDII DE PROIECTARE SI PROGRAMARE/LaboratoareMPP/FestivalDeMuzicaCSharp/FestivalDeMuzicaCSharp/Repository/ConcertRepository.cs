...
public class ConcertRepository : IConcertRepository
{
    ...
    public IEnumerable<ConcertDayDTO> getAllConcertsByDate(DateOnly date)
    {
        ...
        using (IDbCommand command = connection.CreateCommand())
        {
            command.CommandText = "select A.firstname, A.lastname, C.address, C.startTime, C.endTime, C.noSeats, C.concertId, C.name from Concerts C join Artists A on C.artistId = A.artistId where C.date = @date";
            try
            {
                IDbDataParameter dateParameter = command.CreateParameter();
                dateParameter.ParameterName = "@date";
